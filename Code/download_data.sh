...
MaximumThreads=10 # Maximum number of parallel downloads to use
...
for pipeline_ in 1 2 3 4; do
	...
	for preprocessing_ in 1 2 3 4; do
		...
		for site_ in 1 2; do
			...
			python download_abide_preproc.py -d rois_cc200 -p ${pipeline} -s ${preprocessing} -o rois_cc200 -t ${site} &

			python download_abide_preproc_allSubjects.py -d rois_cc200 -p ${pipeline} -s ${preprocessing} -o rois_cc200_allSubjects -t ${site} &
